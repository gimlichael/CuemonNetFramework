...
namespace Cuemon.Web
{
    public partial class GlobalModule
    {
        ...
        protected virtual void HandleHtmlRelatedContentParsing(HttpApplication context, CacheValidator validator, Doer<Stream, Encoding, CacheValidator, Stream> htmlOutputParser)
        {
            ...
            if (EnableCompression && IsValidForCompression(context))
            {
                string contentEncoding = HttpRuntimeUtility.SupportsIisIntegratedPipelineMode
                                                ? context.Response.Headers["Content-Encoding"] ?? GetClientCompressionMethod(context).ToString()
                                                : GetClientCompressionMethod(context).ToString();
            ...
            ParseWriteHtmlRelatedContent(context, content, compression);
        }
        ...
        {
            byte[] output = ParseHttpOutputStream(content, compression);
