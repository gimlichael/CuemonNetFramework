...
namespace Cuemon.Web.Security
{
    ...
    public sealed class JsonWebToken
    {
        ...
        {
            string base64UrlEncodedHeader = StringConverter.ToUrlEncodedBase64(Encoding.UTF8.GetBytes(Header.ToString()));
            string base64UrlEncodedPayload = StringConverter.ToUrlEncodedBase64(Encoding.UTF8.GetBytes(Payload.ToString()));
            StringBuilder token = new StringBuilder();
            token.Append(base64UrlEncodedHeader);
            token.Append(".");
            token.Append(base64UrlEncodedPayload);
            var tokenString = token.ToString();
            return Header.Algorithm == JsonWebTokenHashAlgorithm.SHA256 ? string.Format(CultureInfo.InvariantCulture, "{0}.{1}", tokenString, HmacUtility.ComputeKeyedHash(tokenString, Secret, HmacAlgorithmType.SHA256, Encoding.UTF8).ToUrlEncodedBase64()) : tokenString;
        }
    }
}