...
namespace Cuemon.Web.UI
{
	...
	//[SerializationFilter(Filter.Exclude, typeof(Control), typeof(HttpSessionState), typeof(StateBag), typeof(HttpServerUtility), typeof(HttpApplicationState), typeof()]
	//[SerializationFilter(Filter.Include, typeof(bool), typeof(string), typeof(DateTime), typeof(XsltPageLocalization), typeof(XsltMasterPage), typeof(HttpContextItemCollection), typeof(HttpContextItem))]
	[XmlRoot("Page")]
	public abstract class XsltPage : Page, IXmlSerialization, ISearchEngineOptimizer, IRequiresSessionState, ICacheableHttpHandler
	{
	    ...
	    private DateTime? _xslCompiledTransformCacheRefresh = null;
        ...
        /// <summary>
        /// Finalizes an instance of the <see cref="XsltPage"/> class.
        /// </summary>
        ~XsltPage()
        {
            Dispose(false);
		...
		public HttpContextItemCollection ContextItems
		{
			get
			{
				...
				{
					contextItemsValue = this.IncludeServerVariablesOnSerialization ? new HttpContextItemCollection(this.Context) : new HttpContextItemCollection(this.Context, HttpContextType.Application, HttpContextType.Cookies, HttpContextType.Files, HttpContextType.Form, HttpContextType.QueryString, HttpContextType.Session);
				}
        ...
        //{
        //    return HashUtility.ComputeHash(this.Output, HashAlgorithmType.MD5, true);
        //}
		...
		public virtual CacheValidator GetCacheValidator()
		{
            ...
            {
                CacheValidator styleshettValidator = CacheValidator.Default;
                ...
                CacheValidator xslCompiledTransformRefresh = CacheValidator.Default;

                if (this.StyleSheet != null)
                {
                    styleshettValidator = FileUtility.GetCacheValidator(this.StyleSheet).CombineWith(Thread.CurrentThread.CurrentCulture.LCID);
                }
                ...
                if (_xslCompiledTransformCacheRefresh.HasValue)
                {
                    xslCompiledTransformRefresh = new CacheValidator(_xslCompiledTransformCacheRefresh.Value, ChecksumMethod.Combined);
                }

                _cacheValidator = CacheValidator.GetMostSignificant(styleshettValidator, masterStylesheetValidator, currentPageValidator, configFileValidator, sitemapFileValidator, phraseFileValidator, xslCompiledTransformRefresh);
            }
		...
		private void RenderHelper(HtmlTextWriter writer)
		{
		    try
		    {
                ...
                Stream websiteXml = StreamUtility.CopyStream(this.XmlForRendering, true);
                Stream output = this.Output;
                renderedContent = this.ToString(output, PreambleSequence.Keep, this.Response.ContentEncoding);
                if (EnableDebug) { this.SnapshotOfXmlForDebugging = websiteXml; }
        ...
        private Stream XmlForRendering
        {
            get
            {
                if (_xmlForRendering == null)
                {
                    if (EnableMetadataCaching)
                    {
                        Doer<string, string> computeHash = CachingManager.Cache.Memoize<string, string>(HashUtility.ComputeHash);
                        string rawUrl = this.Request.RawUrl.ToLowerInvariant(); 
			            string cacheKey = this.Website != null ? computeHash(string.Format(CultureInfo.InvariantCulture, "{0}{1}{2}_MetaCache", rawUrl, this.Website.Globalization.CurrentCultureInfo.LCID, this.Website.Globalization.CurrentTimeZone.StandardName)) : computeHash(rawUrl + "_MetaCache");

                        byte[] metacache = CachingManager.Cache.GetOrAdd(cacheKey, CacheGroupName, () =>
                            {
                                return ConvertUtility.ToByteArray(this.Website != null ? this.Website.ToXml() : this.ToXml(false, new XmlQualifiedEntity("Page")));
                            });
                        _xmlForRendering = ConvertUtility.ToStream(metacache);
                    }
                    else
                    {
                        _xmlForRendering = this.Website != null ? this.Website.ToXml() : this.ToXml(false, new XmlQualifiedEntity("Page"));
                    }
                }
                return _xmlForRendering;
            }
        }
		...
		{
		    Doer<string, string> computeHash = CachingManager.Cache.Memoize<string, string>(HashUtility.ComputeHash);
		    string cacheKey = computeHash(string.Concat(this.Request.Url.AbsolutePath.ToLowerInvariant(), "_OutputSettings()"));
		...
		private Stream StyleSheetAsStream
		{
		    get
		    {
                if (_stylesheet == null)
                {
                    if (this.StyleSheetNavigable == null && string.IsNullOrEmpty(this.StyleSheet)) { throw new InvalidOperationException("There is no valid style sheet for the current state of this object."); }
                    byte[] stylesheetBytes = !string.IsNullOrEmpty(this.StyleSheet) ? this.GetStyleSheet(this.StyleSheet) : GetStyleSheet(this.StyleSheetNavigable);
                    _stylesheet = ConvertUtility.ToStream(stylesheetBytes);
                }
                return _stylesheet;
		    }
		}

		private static byte[] GetStyleSheet(IXPathNavigable stylesheet)
		{
			return ConvertUtility.ToByteArray(stylesheet.CreateNavigator().OuterXml);
		}

		private byte[] GetStyleSheet(string stylesheet)
		{
		    Uri stylesheetAsUri;
            Doer<UriScheme[]> getAllUriSchemes = CachingManager.Cache.Memoize<UriScheme[]>(UriUtility.GetAllUriSchemes);
            bool isUri = UriUtility.TryParse(stylesheet, UriKind.Absolute, getAllUriSchemes(), out stylesheetAsUri);
			if (!isUri)
			{
				if (!this.AutoStyleSheetResolving) { stylesheet = this.Server.MapPath(stylesheet); } // assume local references
			}
			else
			{
			    isUri = !stylesheetAsUri.IsFile;
			}

			if (EnableStyleSheetCaching) // only cache XSL stylesheet files if enabled (Note: Style sheet caching should ALWAYS be enabled in a production environment!)
			{
                Doer<string, string> computeHash = CachingManager.Cache.Memoize<string, string>(HashUtility.ComputeHash);
				string cacheKey = computeHash(string.Concat(stylesheet.ToLowerInvariant(), "_GetStyleSheet(string)"));

                Dependency dependency = null;
                if (!isUri)
                {
                    dependency = new FileDependency(Path.GetDirectoryName(stylesheet), Path.GetFileName(stylesheet));
                    dependency.DependencyChanged += XsltFileDependenciesChanged;
                }
                else
                {
                    dependency = new NetDependency(stylesheet);
                }
			    byte[] cachedBytes = CachingManager.Cache.GetOrAdd(cacheKey, CacheGroupName, () => 
                    {
                        return this.ReadStyleSheet(stylesheet, isUri);
                    }, dependency);
        ...
        void XsltFileDependenciesChanged(object sender, DependencyEventArgs e)
        {
            _xslCompiledTransformCacheRefresh = DateTime.UtcNow;
            CachingManager.Cache.Clear(CacheGroupName);
            XsltUtility.ClearXslCompiledTransformCache();
        }

		private byte[] ReadStyleSheet(string stylesheet, bool isUri)
		{
			if (isUri)
			{
				using (WebResponse response = NetUtility.GetResponse(WebRequest.Create(stylesheet)))
				{
					byte[] bytes = ConvertUtility.ToByteArray(StreamUtility.CopyStream(response.GetResponseStream()));
					return bytes;
				}
			}

			using (FileStream stream = new FileStream(stylesheet, FileMode.Open, FileAccess.Read))
			{
		...
		public XPathNodeIterator GetCustomXmlNodes()
		{
		    ...
		    XPathNodeIterator iterator = CachingManager.Cache.GetOrAdd(cacheKey, CacheGroupName, () =>
                {
                ...
                }, new FileDependency(directory, filter));
