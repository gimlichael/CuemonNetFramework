...
namespace Cuemon.Xml
{
	...
	public sealed class SecureXmlObfuscator : XmlObfuscator
	{
		...
		public override Stream Revert(Stream value, Stream mapping)
		{
			...
			try
			{
				XmlDocument document = XmlUtility.CreateXmlDocument(mapping);
				XmlNode mappingNode = document.DocumentElement;
				XmlNode encryptedNode = mappingNode.SelectSingleNode(MappingEncryptedElement);
				if (encryptedNode != null)
				{
					mappingNode.InnerXml = ConvertUtility.ToString(AdvancedEncryptionStandardUtility.Decrypt(Convert.FromBase64String(encryptedNode.InnerText), this.Key, this.InitializationVector), PreambleSequence.Remove, this.Encoding);
				}
		...
		public override Stream CreateMapping()
		{
			...
			try
			{
				XmlDocument document = XmlUtility.CreateXmlDocument(base.CreateMapping());
				XmlNode mappingNode = document.DocumentElement;
				string innerXmlOfMappingNode = mappingNode.InnerXml;
				byte[] innerXmlOfMappingNodeBytes = ConvertUtility.ToByteArray(innerXmlOfMappingNode, PreambleSequence.Remove, this.Encoding);
				XmlElement encryptedNode = document.CreateElement(MappingEncryptedElement);
				encryptedNode.InnerText = Convert.ToBase64String(AdvancedEncryptionStandardUtility.Encrypt(innerXmlOfMappingNodeBytes, this.Key, this.InitializationVector));
