...
namespace Cuemon
{
    ...
    public sealed class Hierarchy<T> : Wrapper<T>, IHierarchy<T>
    {
        ...
        {
            IHierarchy<T> rootItem = EnumerableUtility.FirstOrDefault(Hierarchy.AncestorsAndSelf(newItem));
            IEnumerable<IHierarchy<T>> allItems = Hierarchy.DescendantsAndSelf(rootItem);
            return EnumerableUtility.Count(allItems);
        }
    ...
    public static class Hierarchy
    {
        ...
        public static IHierarchy<T> Root<T>(IHierarchy<T> node)
        {
            ...
            return EnumerableUtility.FirstOrDefault(AncestorsAndSelf(node));
        }
