    ...
    public static class HmacUtility
    {
        ...
        public static HashResult ComputeKeyedHash(object[] values, byte[] sharedKey, HmacAlgorithmType algorithmType)
        {
            ...
            if (values.Length == 1) // only one object is needed for hashing; avoid the extra overhead by combining
            {
                ...
                return ComputeKeyedHash(HashUtility.SerializeObject(values[0]), sharedKey, algorithmType);
            }
            List<Stream> streams = new List<Stream>();
            foreach (object value in values)
            {
                if (value != null) { streams.Add(HashUtility.SerializeObject(value)); }
            }
            return ComputeKeyedHash(StreamUtility.CombineStreams(streams.ToArray()), sharedKey, algorithmType);
        }
        ...
        {
            if (algorithmType > HmacAlgorithmType.RIPEMD160 || algorithmType < HmacAlgorithmType.SHA1) { throw new ArgumentOutOfRangeException(nameof(algorithmType), "Specified argument was out of the range of valid values."); }
            ...
            switch (algorithmType)
            {
                ...
                case HmacAlgorithmType.SHA512:
                    ...
                    break;
                case HmacAlgorithmType.RIPEMD160:
                    algorithm = new HMACRIPEMD160(sharedKey);
                    break;
                default:
                    algorithm = new HMACSHA1(sharedKey, true);
                    break;
            }
