...
namespace Cuemon.Security
{
	...
	public static class SecurityUtility
	{
		...
		{
            return CreateEncryptedSecurityToken(token, ConvertUtility.ToByteArray(securityKey, PreambleSequence.Remove, encoding), ConvertUtility.ToByteArray(securityInitializationVector, PreambleSequence.Remove, encoding));
		}
		...
		public static byte[] CreateEncryptedSecurityToken(SecurityToken token, byte[] securityKey, byte[] securityInitializationVector)
		{
            ...
            byte[] securityToken = AdvancedEncryptionStandardUtility.Encrypt(ConvertUtility.ToByteArray(token.ToString(), PreambleSequence.Remove, Encoding.UTF8), securityKey, securityInitializationVector);
		...
		{
            return ParseEncryptedSecurityToken(securityToken, ConvertUtility.ToByteArray(securityKey, PreambleSequence.Remove, encoding), ConvertUtility.ToByteArray(securityInitializationVector, PreambleSequence.Remove, encoding));
		}
		...
		{
			return SecurityToken.Parse(ConvertUtility.ToString(AdvancedEncryptionStandardUtility.Decrypt(securityToken, securityKey, securityInitializationVector), PreambleSequence.Remove, Encoding.UTF8));
		}
	}
}
