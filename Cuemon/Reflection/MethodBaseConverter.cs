...
namespace Cuemon.Reflection
{
    ...
    public static class MethodBaseConverter
    {
        ...
        public static MethodBase FromType(Type caller, string memberName, BindingFlags flags = BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public)
        {
        ...
        /// <param name="flags">A bitmask comprised of one or more <see cref="BindingFlags"/> that specify how the search is conducted.</param>
        /// <returns>An object representing the method that matches the specified requirements, if found; otherwise, <c>null</c>.</returns>
        public static MethodBase FromType(Type caller, Type[] types, string memberName, BindingFlags flags = BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public)
        {
            Validator.ThrowIfNull(caller, nameof(caller));
            Validator.ThrowIfNullOrEmpty(memberName, nameof(memberName));
            var methods = new List<MethodInfo>(EnumerableUtility.Where(caller.GetMethods(flags), info => info.Name.Equals(memberName)));
