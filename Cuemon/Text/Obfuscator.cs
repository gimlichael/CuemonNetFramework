...
namespace Cuemon.Text
{
    ...
    public abstract class Obfuscator
    {
        ...
        {
            get { return _permutationCharacters ?? (_permutationCharacters = this.InitializePermutationCharacters()); }
        }
        ...
        {
            return HashUtility.ComputeHash(value, HashAlgorithmType.SHA1, this.Encoding);
        }
        
        private string GeneratePermutationValue()
        {
            StringBuilder values = new StringBuilder(this.CurrentCombinationLength);
            for (byte i = 0; i < this.CurrentCombinationLength; i++)
            {
                values.Append(this.PermutationCharacters[NumberUtility.GetRandomNumber(0, this.PermutationCharacters.Count, RandomSeverity.Strong)].ToString());
            }
        ...
        {
            this.HandlePermutationCalculation();
            string obfuscatedValue = this.GenerateObfuscatedValue(this.GeneratePermutationValue());
            lock (this.ObfuscatedValues)
            {
                this.ObfuscatedValues.Add(obfuscatedValue);
            }
        ...
        private string GenerateObfuscatedValue(string value)
        {
            lock (this.ObfuscatedValues)
            {
                if (!this.ObfuscatedValues.Contains(value))
                {
                    return value;
                }
            }
            return this.GenerateObfuscatedValue(this.GeneratePermutationValue());
        }

        private void HandlePermutationCalculation()
        {
            if (this.CurrentPermutationSize == this.ObfuscatedValues.Count)
            {
                lock (locker)
                {
                    if (this.CurrentPermutationSize == this.ObfuscatedValues.Count)
                    {
                        this.IncrementCurrentCombinationLength();
                        this.IncreaseCurrentPermutationSize();
                    }
                }
            }
        }
        ...
        {
            _currentPermutationSize = (int)Math.Round((NumberUtility.Factorial(this.PermutationCharacters.Count) / NumberUtility.Factorial(this.PermutationCharacters.Count - this.CurrentCombinationLength)));
        }
