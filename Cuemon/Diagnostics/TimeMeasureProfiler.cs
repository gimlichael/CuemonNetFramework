...
namespace Cuemon.Diagnostics
{
    ...
    public class TimeMeasureProfiler : Profiler
    {
        ...
        {
            StringBuilder result = new StringBuilder(string.Format(CultureInfo.InvariantCulture, "{0} took {1:D2}:{2:D2}:{3:D2}.{4:D3}{5:D3}{6:D3} to execute.", Member,
                Elapsed.Hours,
                Elapsed.Minutes,
                Elapsed.Seconds,
                Elapsed.Milliseconds,
                Int32Converter.ToMicroseconds(Elapsed),
                Int32Converter.ToNanoseconds(Elapsed)));
            if (Data.Count > 0)
            {

                result.Append(" Parameters: { ");
                result.Append(StringConverter.ToDelimitedString(Data, ", ", pair => string.Format(CultureInfo.InvariantCulture, "{0}={1}", pair.Key, StringConverter.FromObject(pair.Value))));
                result.Append(" }");
            }
