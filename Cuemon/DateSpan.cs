...
namespace Cuemon
{
	...
	public struct DateSpan : IEquatable<DateSpan>
	{
		...
		public DateSpan(DateTime start, DateTime end, Calendar calendar) : this()
		{
			...
			this.Highest = DateTimeUtility.GetHighestValue(start, end);
			this.Lowest = DateTimeUtility.GetLowestValue(start, end);
			this.Calendar = calendar;

            int totalMonths, deltaMonths;
            this.GetMonths(out deltaMonths, out totalMonths);

			TimeSpan timespan = (this.Highest - this.Lowest);
			this.Hours = timespan.Hours;
			this.TotalHours = (long)timespan.TotalHours;
			this.Milliseconds = timespan.Milliseconds;
			this.TotalMilliseconds = (long)timespan.TotalMilliseconds;
			this.Minutes = timespan.Minutes;
			this.TotalMinutes = (long)timespan.TotalMinutes;
		    this.Months = deltaMonths;
            this.TotalMonths = totalMonths;
		    this.Days = this.Highest.Day;
			this.TotalDays = (int)Math.Floor(timespan.TotalDays);
			this.Seconds = timespan.Seconds;
			this.TotalSeconds = (long)timespan.TotalSeconds;
			this.Ticks = timespan.Ticks;
			this.Years = this.GetYears(this.Highest, this.Lowest);
		}
		#endregion
		...
		public int GetWeeks()
		{
			TimeSpan range = this.Highest.Subtract(this.Lowest);
			int totalDays = 0;
			if (range.Days <= 7)
			{
				totalDays = this.Lowest.DayOfWeek > this.Highest.DayOfWeek ? 2 : 1;
			}
			if (totalDays == 0) { totalDays = range.Days - 7 + (int)this.Lowest.DayOfWeek; }
		...
		{
			return this.Highest.GetHashCode() ^ this.Lowest.GetHashCode() ^ this.Calendar.GetHashCode();
		}
		...
		{
			if ((this.Highest != other.Highest) || (this.Calendar != other.Calendar)) { return false; }
		...
		{
			return Parse(start, end, CultureInfo.InvariantCulture.LCID);
		}
		...
		/// <returns>A <see cref="DateSpan"/> that corresponds to <paramref name="start"/> and <paramref name="end"/> of the interval.</returns>
		public static DateSpan Parse(string start, string end, int lcid)
		{
		...
		{
			totalMonths = 0;
            deltaMonths = this.Highest.Month;
			for (int year = this.Lowest.Year; year < this.Highest.Year; year++)
			{
				totalMonths += this.Calendar.GetMonthsInYear(year);
			}
		...
		{
			return string.Format(CultureInfo.InvariantCulture, "{0}:{1:D2}:{2:D2}:{3:D2}:{4:D2}:{5:D2}.{6}", this.Years, this.Months, this.Days, this.Hours, this.Minutes, this.Seconds, this.Milliseconds);
		}
		#endregion
